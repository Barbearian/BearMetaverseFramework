...
namespace Bear{
	public class Dependencies :Loadable
	{
		...
		{
			if (!Loader.GetDependencies(pathOrURL, out var info, out var infos))
			{
			...
			_mainBundle = Loader.LoadBundleInternally(info);
			_bundles.Add(_mainBundle);
			if (infos == null || infos.Length <= 0) return;

			foreach (var item in infos) _bundles.Add(Loader.LoadBundleInternally(item));
		}
		...
		protected override void OnUnload()
		{
			...
			_mainBundle = null;
			Loader.DependenciesCache.Remove(pathOrURL);
		}
